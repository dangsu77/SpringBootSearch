...
import com.shinez.springboot.common.WNConstants.*;

import static com.shinez.springboot.common.WNConstants.*;
import static com.shinez.springboot.common.WNUtils.*;

/**
 * 
 * <pre>
 * <p> Title: WNSearch </p>
 * <p> Description: 검색페이지에서 사용되는 메소드를 정의한 클래스</p>
 * </pre>
 *
 * @author WISEnut
 * @created: 2014. 5. 27.
...
public class WNSearch {
	...
	public WNSearch(String searcherId, boolean isDebug, boolean isUidSrch, 
	    ...
	    this.wncol = new WNConstants().new WNCollection();
	...
	private int search(String query, boolean isRealQuery, String[] userInfo, int hiSum, int useOrResult, int connectionOpt, boolean useSuggestedQuery) {
		...
		//검색결과를 얻는다.
		if(!isUidSrch) {
			if(resultType == WNConstants.USE_RESULT_XML) {
				ret = common.receiveSearchQueryResultAsXml(this.connectionOpt);
			}else if(resultType == WNConstants.USE_RESULT_JSON) {
			...
			} else if(resultType == WNConstants.USE_RESULT_DUPLICATE_STRING) {
				ret = common.recvDuplicateDocumentsResult(this.connectionOpt);
			} else if(resultType == WNConstants.USE_RESULT_DUPLICATE_XML) {
				ret = common.recvDuplicateDocumentsResultAsXml(this.connectionOpt);
			} else if(resultType == WNConstants.USE_RESULT_DUPLICATE_JSON) {
